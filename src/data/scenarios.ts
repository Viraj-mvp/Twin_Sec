...
export const SECTOR_TOPOLOGIES: Record<SectorId, { nodes: Node[]; edges: Edge[] }> = {
  power: {
    nodes: [
      {
        id: "g1",
    ...
    nodes: [
      {
        id: "bms",
        label: "BMS-01",
        kind: "Building Mgmt Server",
        x: 10,
        y: 50,
        ring: 0,
        vendor: "TRIDIUM NIAGARA 4",
        firmware: "v4.11",
        exposure: "L3.5/DMZ",
        affects: ["HVAC", "Lighting", "Access"],
      },
      {
        id: "hvac",
        label: "CHILLER-A",
        kind: "Centrifugal Chiller",
        x: 30,
        y: 25,
        ring: 1,
        vendor: "YORK YK",
        firmware: "v3.2",
        exposure: "L2/OT",
        affects: ["Cooling loop", "Server floor"],
      },
      {
        id: "ahu",
        label: "AHU-04",
        kind: "Air Handling Unit",
        x: 30,
        y: 75,
        ring: 1,
        vendor: "CARRIER 39M",
        firmware: "v2.0",
        exposure: "L2/OT",
        affects: ["Floors 12–24"],
      },
      {
        id: "elev",
        label: "ELEV-CTRL",
        kind: "Elevator Dispatch",
        x: 52,
        y: 25,
        ring: 2,
        vendor: "OTIS Compass 360",
        firmware: "v5.0",
        exposure: "L2/OT",
        affects: ["Bank A & B elevators"],
      },
      {
        id: "locks",
        label: "LOCK-SYS",
        kind: "Access Control Panel",
        x: 52,
        y: 75,
        ring: 2,
        vendor: "HID Aero X1100",
        firmware: "v2.9",
        exposure: "L2/OT",
        affects: ["Turnstiles", "Executive floor"],
      },
      {
        id: "access",
        label: "ACCESS-SVR",
        kind: "Card Reader Server",
        x: 74,
        y: 50,
        ring: 3,
        vendor: "GENETEC Synergis",
        firmware: "v5.10",
        exposure: "L3/IT",
        affects: ["Badge database", "Logins"],
      },
      {
        id: "fire",
        label: "FIRE-ALARM",
        kind: "Life Safety Panel",
        x: 90,
        y: 25,
        ring: 4,
        vendor: "NOTIFIER NFS2-3030",
        firmware: "v4.0",
        exposure: "L1/SAFETY",
        affects: ["Stairwell pressurization"],
      },
      {
        id: "nocgw",
        label: "NOC-GW",
        kind: "Midtown Fiber Gateway",
        x: 90,
        y: 75,
        ring: 4,
        vendor: "FORTINET 60F",
...
export function getScenarioData(sector: SectorId): ScenarioData {
  ...
  const topology = SECTOR_TOPOLOGIES[sector] ?? { nodes: DEFAULT_NODES, edges: EDGES };
  let nodes = topology.nodes.map((n) => ({ ...n }));
